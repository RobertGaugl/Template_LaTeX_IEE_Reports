...
\newcommand{\reportauthors}{R. Gaugl, A. Author, B. Author}
\newcommand{\reportpartners}{TU Graz, Partner Company 1}
\newcommand{\reportversion}{1.0}
\newcommand{\reportcity}{Graz}

% For author page
\newcommand{\reportauthorlist}{%
    \reportauthorcompany{TU Graz}
    \reportauthorentry{Robert Gaugl}{Assistant Professor}{example@email.com}
    \reportauthorentry{Alexa Author}{Project Assistant}{alexa.author@email.com}

    \reportauthorcompany{Partner Company 1}
    \reportauthorentry{Brian Author}{Team Leader}{brian.author@email.com}
}
...
% Comment out the following line to disable the disclaimer.
% \newcommand{\ieedisclaimertext}{INTERNAL USE ONLY -- NOT FOR PUBLICATION OR DISTRIBUTION}
...
\ieeline
\sideIcon{\fontsize{36}{40}\selectfont\color{blue}\faIcon{sitemap}}{center}
...
\ieeline
\sideIcon{\fontsize{36}{40}\selectfont\color{blue}\faIcon{paintbrush}}{center}
...
\ieeline
\sideIcon{\fontsize{36}{40}\selectfont\color{blue}\faIcon{recycle}}{center}
{\Large\color{subtitle}\textbf{Reusable elements make writing more efficient}}

The template provides a small set of reusable components such as \texttt{coloredBlock}, \texttt{sideBlock}, \texttt{sideNote},   \texttt{sideFigure}, and more. These elements help authors structure reports consistently without redefining layout choices for each new project.
...
\begin{sideBlock}[yellow]{\faIcon{bolt}~~Sidebar usage}
...
\begin{coloredBlock}[yellow]{\faIcon{bolt}~~Recommendation}
...
\begin{sideBlock}[yellow]{\faIcon{bolt}~~Sidebar content}
...
Short results, assumptions, or notes are good candidates for a side note.
\end{sideNote}
...
\begin{coloredBlock}[turquoise]{\faIcon[regular]{lightbulb}~~Design tip}
...
\begin{sideBlock}[yellow]{\faIcon{bolt}~~Icon style}
...
\begin{coloredBlock}[turquoise]{\faIcon[regular]{lightbulb}~~Design tip}
A subtle grey background (e.g. \texttt{greyLight}) can improve the visual integration of figures. It separates the figure from the main text without drawing too much attention.
\end{coloredBlock}

\begin{sideNote}[blue]{}
\textcolor{blue}{\Huge\bfseries\raisebox{0.4ex}{\Large +}\,\SI{140}{\percent}}\\[0.2em]
PV capacity\\[1em]
\textcolor{blue}{\Huge\bfseries\raisebox{0.4ex}{\Large +}\,\SI{62}{\percent}}\\[0.2em]
Wind capacity\\[1em]
...
\begin{figure}[h]
    ...
    \colorbox{greyLight}{%
        \begin{minipage}{\linewidth}
            ...
            ]{figures/installed_capacity_2022-2025.pdf}
...
\sideFigure[width=0.85\linewidth]{figures/battery_storage.pdf}{Example of a side figure placed in the left sidebar.}{}{center}

The command \texttt{\textbackslash sideFigure} allows authors to place a compact figure in the sidebar. This is useful for small icons, supporting charts, or visual references that complement the nearby paragraph.

A typical usage example is:

\begin{coloredBlock}[grey]{}
\begin{verbatim}
\sideFigure[width=0.85\linewidth]{example-image}
...
\bottomrule
\end{tabularx}
}{Example of a compact side table.}

A side table can also be placed in the sidebar using \texttt{\textbackslash sideTable}, but only for very compact tabular content.
...
Use \texttt{\textbackslash num\{\}} for plain numbers,\texttt{\textbackslash si\{\}} for units without values and \texttt{\textbackslash SI\{\}\{\}} for values with units. This keeps the formatting of the entire report consistent.
\end{coloredBlock}
...
\begin{itemize}
    \item A plain number \texttt{\textbackslash num\{12345.67\}}: \num{12345.67} 
    \item A unit only \texttt{\textbackslash si\{\textbackslash watt\}}: \si{\watt}
    \item A value with a unit \texttt{\textbackslash SI\{1200\}\{\textbackslash mega\textbackslash watthour\}}: \SI{1200}{\mega\watthour}
    \item A percentage value \texttt{\textbackslash SI\{12.5\}\{\textbackslash percent\}}: \SI{12.5}{\percent}
...
Even when language settings are available, the report should still use a consistent notation for all numerical values and units.
\end{sideNote}
...
\begin{sideBlock}[yellow]{\faIcon{bolt}~~\texttt{sideBlock} length}
...
\begin{sideBlock}[green]{\faIcon[regular]{compass}~~Quick guide}
Use \texttt{\textbackslash cite\{\}} or \texttt{\textbackslash citep\{\}} when you only want the reference number. Use \texttt{\textbackslash citet\{\}} or \texttt{\textbackslash textcite\{\}} when the authors should appear as part of the sentence.
\end{sideBlock}
...
Figures and tables should be labeled and referenced from the text. This makes the document easier to navigate and avoids ambiguity. For example, the full-width illustration introduced earlier can be referenced as \autoref{fig:full-width-figure}.
...
\begin{coloredBlock}[turquoise]{\faIcon[regular]{lightbulb}~~Practical tips}
